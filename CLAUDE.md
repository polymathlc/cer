...
- `common-mistakes.html` — "Common Mistakes Tracker": students log exam mistakes (question / wrong answer / correct answer / explanation), stored per-user in Firestore (`commonMistakes/{uid}/mistakes`) on the shared `mathgen--app` project with Google sign-in. Includes a flashcard practice mode with a streak-based mastery system (3 consecutive correct recalls = mastered) and a 6-item starter set seeded from the original screenshot. Has its own `APP_VERSION` badge (`#versionBadge`, shown only to the admin email) — bump it on every change to this file.
- `bar-model.html` — "Bar Model Studio" PSLE maths app: students get a generated (or typed/dictated/photographed) word problem, draw a bar model on an SVG canvas, and submit it for AI marking. Uses the SAME AI marking stack as `index.html`: Firebase AI Logic on the shared `mathgen--app` project, App Check (reCAPTCHA v3), `gemini-3.5-flash` with `thinkingBudget: 0` + JSON response mode, and the tolerant `_parseAIJson`/`_repairAIJson` parser (keep these in sync with `index.html`). Optional admin-only ChatGPT engine (key in localStorage) falls back to Gemini on failure. Its version badge (`#versionBadge`, admin-only) is hard-coded in the HTML — bump it on every change to this file.
