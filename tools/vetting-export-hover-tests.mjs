...
function harness() {
  ...
  const factory = new Function('document','window','setTimeout','clearTimeout','_canAuthor','buildWorksheetHtml','_wsWritePreview','actions', `
    ...
    const _wnyCachedNotes=()=>({cached:true}), wnyPrintOn=()=>false, akxPrintOn=()=>true, lobPrintLines=()=>2;
...
test('hover renders the current Vetting copy with the export options and isolated A4 frame', () => {
  ...
  assert.deepEqual(r.opts,{frontHtml:'',plainNumbers:true,noStudentFields:true,whyNotes:{cached:true},answerKeyExtras:true,loBox:2});
