...
try{for(const file of files){
 ...
 const name=u.pathname.slice(1);if(!files.concat('science-feed-bridge.js','spire-svg-art.js').includes(name))return route.abort();return route.fulfill({contentType:name.endsWith('.js')?'application/javascript':'text/html',body:fs.readFileSync(new URL('../'+name,import.meta.url),'utf8')});});
